...
| Logs REST and SSE are not guild-scoped                                  |     **High** |       High |            High | `/logs` requires only authentication and accepts no guild at `src/core/api/logs.endpoints.ts:20-39`. Authorization skips routes without `:guildId` at `src/core/api/authorization.middleware.ts:13-16`. SSE forwards all entries after session validation at `src/core/api/server.ts:91-129`.            |
...
| Issue                                                                                               |     Severity | Complexity | Regression risk | Evidence                                                                                                                                                 |
| --------------------------------------------------------------------------------------------------- | -----------: | ---------: | --------------: | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Current SDK acceptance suite fails                                                                  | **Critical** |     Medium |            High | `npm run plugin-sdk:check` currently passes 9/10 tests; failure is at `tests/plugin-sdk/phase-10.test.js:18`.                                            |
| SDK harness does not process declarative `instance.events`                                          |     **High** |     Medium |            High | Template declares events in the plugin instance; harness only records `context.events.on()` registrations at `packages/plugin-sdk/src/testing.ts:11-25`. |
| Harness does not enforce manifest capabilities                                                      |     **High** |       High |            High | Commands, events, API, and health registrations are accepted without checking declared capabilities.                                                     |
| Harness cleanup is weaker than production lifecycle semantics                                       |   **Medium** |       High |            High | Stop clears a registrations array rather than exercising registration disposers, timeout/failure order, or leak accounting.                              |
| Generated project is not fully installed, checked, built, tested, and packed as one acceptance path |     **High** |       High |          Medium | Generator test checks output and overwrite refusal; packed-consumer test packages the SDK rather than a fresh generated plugin.                          |
| Custom semver support is incomplete                                                                 |   **Medium** |     Medium |            High | SDK validation implements a limited custom range grammar rather than full npm-compatible semantics.                                                      |
| Documentation snippets are not compiled                                                             |   **Medium** |     Medium |          Medium | Tests verify file/link/string presence only.                                                                                                             |
| API-reference drift check is omitted from `plugin-sdk:check`                                        |   **Medium** |        Low |          Medium | `plugin-sdk:api:check` is a separate script in `package.json`.                                                                                           |
| Phase 10 evidence says SDK tests passed despite current failure                                     |     **High** |        Low |            High | `docs/phases/PHASE-10-EVIDENCE.md:21` contradicts the reproducible test result.                                                                          |
| Public example/template event behavior and harness behavior disagree                                |     **High** |     Medium |            High | The failing test expects an event registration but receives only two command registrations.                                                              |
...
| Platform-wide logs exposed to any authenticated dashboard user                  |        **High** |        High |            High |
