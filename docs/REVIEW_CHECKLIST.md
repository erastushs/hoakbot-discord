...
- [x] **Phase:** Work matches [Phase 03](phases/PHASE-02-PLUGIN-CORE.md), its scope, deliverables, acceptance criteria, testing, and rollback plan; Phase 03 and later helper APIs were not started.
- [x] **Acceptance criteria:** Phase 03 criteria are covered by `tests/unit/built-in-plugin-catalog.test.ts` and the seven per-plugin migration suites: exact General -> Logging -> Welcome -> Goodbye -> Voice -> Moderation -> Shrine order; parity snapshots/projections; single ownership; idempotent startup and cleanup; preserved services; complete API/dashboard metadata; and independent disabled-flag rollback.
