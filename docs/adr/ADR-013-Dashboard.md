...
## Status
Accepted for the 4.0.0 implementation plan. Supersedes legacy ADR-005 API Convention and ADR-008 Dashboard Architecture for dashboard/API compatibility decisions.
...
## Decision
The dashboard remains an API-only, metadata-driven React client. Plugin registry records are projected into existing module payloads and may also be exposed through additive aliases. Existing `/api/v1/modules`, dashboard URLs, response envelopes, OAuth sessions, CSRF, authorization, guild isolation, CORS, rate limiting, security audit, and authenticated guild-scoped SSE live updates remain supported. Dashboard live updates use `EventSource`; WebSocket transport is not part of the v4 baseline dashboard contract.
...
## Related ADRs
ADR-011 Plugin System, ADR-012 Configuration, ADR-014 Command Discovery. Legacy ADR-005 and ADR-008 are archived under `docs/archive/legacy-architecture/adr/`.
