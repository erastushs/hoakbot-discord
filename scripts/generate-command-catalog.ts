...
export function generateCommandCatalog(root: string): string {
  ...
  const files = locations.flatMap((location) => readdirSync(resolve(root, location)).filter((file) => file.endsWith('.command.ts')).map((file) => `${location}/${file}`)).sort();
  ...
  const entries = sources.map(({ file, source }, index) => {
    ...
    const owner = file.includes('/general/') ? 'builtin.general' : file.includes('/moderation/') ? 'builtin.moderation' : null;
    if (!owner) throw new Error(`Command source is outside an approved owner location: ${file}`);
    const factory = source.includes('constructor(') ? className.replace(/Command$/, '').replace(/[A-Z]/g, (value, offset) => `${offset ? '-' : ''}${value.toLowerCase()}`) : 'none';
    return { file, className, owner, factory, alias: `Command${index}` };
  });
  const hash = createHash('sha256').update(sources.map(({ file, source }) => `${file}\0${source}`).join('\0')).digest('hex');
  const imports = entries.map(({ file, className, alias }) => `import { ${className} as ${alias} } from '../${file.slice(4, -3)}.js';`).join('\n');
  const catalog = entries.map(({ file, owner, factory, alias }) => `  Object.freeze({ source: '${file}', owner: '${owner}', factory: '${factory}', command: ${alias} }),`).join('\n');
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(import.meta.filename)) {
  ...
  if (process.argv.includes('--check')) {
    if (!commandCatalogIsCurrent(root)) throw new Error('Generated command catalog is stale. Run npm run generate:commands.');
