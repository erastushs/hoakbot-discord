...
describe('Voice configuration API pipeline', () => {
  it('exposes Voice manifest, metadata, and guild setting values', async () => {
    const provider: IConfigProvider = {
      get: vi.fn(),
      getMany: vi.fn(async () => ({ 'voice.volume': 0.8 })),
      getDefaults: vi.fn(),
      set: vi.fn(),
      setMany: vi.fn(),
    ...
    await expect(router.handle({ method: 'GET', path: '/api/v1/modules' })).resolves.toMatchObject({
      success: true,
      data: { manifests: [expect.objectContaining({ id: 'voice' })] },
    });

    await expect(
      router.handle({ method: 'GET', path: '/api/v1/modules/voice/settings' }),
      ...
      data: {
        settings: expect.arrayContaining([
          expect.objectContaining({
            key: 'voice.volume',
            validationSchema: expect.objectContaining({ minimum: 0, maximum: 2 }),
          }),
          expect.objectContaining({
    ...
    await expect(
      ...
      data: {
        guildId: 'guild-1',
        settings: expect.arrayContaining([
          expect.objectContaining({ key: 'voice.volume', value: 0.8 }),
          expect.objectContaining({ key: 'voice.defaultSound', value: 'hoak' }),
        ]),
      },
    });
  });

  it('validates Voice writes through the settings registry before persisting', async () => {
    const setMany = vi.fn();
    ...
    await expect(
      router.handle({
        method: 'PATCH',
        path: '/api/v1/guilds/guild-1/settings',
        body: { settings: { 'voice.volume': 1.2 } },
      }),
    ).resolves.toMatchObject({
      success: true,
